...
export default function ImageConvertPage() {
  ...
  const label = FORMATS.find((f) => f.value === targetFormat)?.label ?? targetFormat.toUpperCase();

  return (
    <ToolLayout title="Image Format Converter" description="Convert images between JPG, PNG, WebP, and SVG formats instantly.">
      {state === "idle" && (
        <div className="space-y-4">
          ...
          <UploadBox
            ...
            acceptLabel="JPG, PNG, WebP, SVG"
            onFile={handleFile}
          />
        </div>
      )}
      {state === "processing" && <ProgressBar label={`Converting to ${label}...`} />}
      {state === "done" && result && file && (
        <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border)", background: "var(--card)" }}>
          ...
          <div className="p-4 flex justify-center">
            ...
            <img src={result.url} alt="Converted" className="max-w-full rounded-xl object-contain"
